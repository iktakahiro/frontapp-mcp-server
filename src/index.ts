...
server.tool(
  ...
  'Get conversations from a Front inbox',
  {
    inboxId: z
      ...
      .optional()
      .describe(
        'Front Inbox ID (uses DEFAULT_INBOX_ID env var if not provided)',
      ),
    limit: z
      .number()
      .min(1)
  ...
  async ({ inboxId, limit, query, onlyUnresolved, status }) => {
    ...
    if (!resolvedInboxId) {
      throw new Error(
        'inboxId parameter or DEFAULT_INBOX_ID environment variable is required',
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Front API error: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  },
);
